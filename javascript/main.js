// 🎯 CLASSIFIED v7.0 - Fixed Module Loading System
// 
// This version properly handles module loading and ensures demo data is available
...
(function() {
    ...
    function checkDependencies() {
        return window.AppState && window.MockData && window.AppHelpers;
    }
    
    // Initialize CLASSIFIED after dependencies are ready
    function initializeApp() {
        // Create CLASSIFIED object with loaded modules
        window.CLASSIFIED = {
            // Import state from AppState module
            state: window.AppState ? { ...window.AppState.state } : {
                ...
                config: {
                    enableDemoData: true,
                    minUsersBeforeDemo: 6
                }
            },
            
            // Import data from MockData module
            data: window.MockData ? { ...window.MockData.data } : {
                users: [],
                demoUsers: [],
                restaurants: [],
                activities: [],
            ...
            // Import helpers
            helpers: window.AppHelpers || {},
            
            // Initialize App
            init() {
                console.log('🚀 Starting CLASSIFIED v7.0 (Fixed Module Loading)...');
                
                // Initialize demo data if MockData is available
                if (window.MockData && window.MockData.data) {
                    this.data = { ...window.MockData.data };
                    console.log('✅ Demo data loaded:', {
                        demoUsers: this.data.demoUsers ? this.data.demoUsers.length : 0,
                        restaurants: this.data.restaurants ? this.data.restaurants.length : 0
                    });
                }
            ...
            populateGuestUserFeed() {
                ...
                // Show first 3 demo users to guests
                const guestUsers = (this.data.demoUsers || []).slice(0, 3);
                
                guestUsers.forEach((user, index) => {
                    const feedItem = this.createUserFeedItem({
                        ...user,
                ...
                signupCard.innerHTML = `
                    <div style="background: linear-gradient(135deg, #FFD700, #FF6B6B); padding: 30px; border-radius: 20px; text-align: center; margin: 20px 0;">
                        ...
                        <p style="margin: 0 0 20px 0; opacity: 0.9;">Join ${guestUsers.length}+ travelers already using CLASSIFIED</p>
            ...
            async populateRestaurantFeed() {
                ...
                try {
                    ...
                    if (restaurants.length === 0 && this.data.restaurants) {
            ...
            renderRestaurantFeed(restaurants, storiesContainer, feedContainer) {
                if (!restaurants || restaurants.length === 0) {
                    console.warn('⚠️ No restaurants to display');
                    if (feedContainer) {
                        feedContainer.innerHTML = '<div style="text-align: center; padding: 40px; opacity: 0.7;">No restaurants available</div>';
                    }
                    return;
                }
            ...
            async populateActivityFeed() {
                ...
                try {
                    ...
                    // Use demo data as fallback
                    if (activities.length === 0 && this.data.activities) {
            ...
            renderActivityFeed(activities, storiesContainer, feedContainer) {
                if (!activities || activities.length === 0) {
                    ...
                    if (feedContainer) {
                        feedContainer.innerHTML = '<div style="text-align: center; padding: 40px; opacity: 0.7;">No activities available</div>';
                    }
                    return;
                }
            ...
            // 👥 User Feed Management - FIXED VERSION
            async populateUserFeed() {
                ...
                try {
                    ...
                    // Fetch real users from Firebase if authenticated
                    if (this.state.isAuthenticated && window.db) {
                    ...
                    // Add demo users if enabled and needed
                    if (this.state.config.enableDemoData && users.length < this.state.config.minUsersBeforeDemo) {
                        const demoUsers = this.data.demoUsers || [];
                        const neededDemo = this.state.config.minUsersBeforeDemo - users.length;
                        const demoToAdd = demoUsers.slice(0, neededDemo).map(user => ({
                            ...user,
                            id: `demo_${user.name.toLowerCase().replace(/\s/g, '_')}`,
                            uid: `demo_${user.name.toLowerCase().replace(/\s/g, '_')}`,
                            isDemo: true
                        ...
                        users = [...users, ...demoToAdd];
                        console.log(`🎭 Added ${demoToAdd.length} demo users`);
                    }
            ...
            filterUsers(filter) {
                ...
                // In a real implementation, you'd re-query Firebase with the filter
                console.log(`Filter ${filter} selected - would filter users accordingly`);
            },
            ...
            createUserFeedItem(user, index) {
                ...
                // Add demo class if it's a demo user
                if (user.isDemo) {
                    feedItem.classList.add('demo-user');
                }
                ...
                feedItem.innerHTML = `
                    <div class="user-status-badges">
                        ...
                        ${user.isDemo ? '<div class="status-badge" style="background: rgba(255,215,0,0.2); color: #FFD700;">🎭 Demo</div>' : ''}
                    </div>
                    ...
                    <div class="user-info">
                        ...
                        <div class="user-actions">
                            <button class="action-btn pass-btn" onclick="event.stopPropagation(); CLASSIFIED.handleUserAction('pass', '${user.name}', ${user.isDemo})">✕ Pass</button>
                            <button class="action-btn chat-btn" onclick="event.stopPropagation(); CLASSIFIED.handleUserAction('like', '${user.name}', ${user.isDemo})">💬 Chat</button>
                            <button class="action-btn super-btn" onclick="event.stopPropagation(); CLASSIFIED.handleUserAction('superlike', '${user.name}', ${user.isDemo})">⭐ Super</button>
                        </div>
                    </div>
                `;
                return feedItem;
            ...
            openProfileFromChat() {
                ...
                if (this.state.currentChatUser) {
                    ...
                    if (chatName) {
                        // Check both real and demo users
                        const allUsers = [...(this.data.users || []), ...(this.data.demoUsers || [])];
                        user = allUsers.find(u => u.name.trim() === chatName);
                        if (user) {
            ...
            // Modified handleUserAction to handle demo users
            async handleUserAction(action, userName, isDemo = false) {
                console.log(`👆 ${action} action for ${userName} (demo: ${isDemo})`);
                
                if (!this.state.isAuthenticated) {
                    alert('Please sign up to interact with users!');
                    this.showRegister();
                    return;
                }
                
                const targetUser = this.state.currentViewedUser || 
                    [...(this.data.users || []), ...(this.data.demoUsers || [])].find(u => u.name === userName);
                ...
                if (isDemo) {
                    ...
                    if (action === 'like' || action === 'superlike') {
                        ...
                        setTimeout(() => {
                            alert(`🎉 It's a match! ${userName} liked you back!\n\n(This is a demo user - invite real friends to start connecting!)`);
            ...
            showUserGrowthFeatures() {
                ...
                if (restaurantStories && !document.querySelector('.referral-banner')) {
        ...
        // Initialize the app
        window.CLASSIFIED.init();
    }
    
    // Wait for dependencies with timeout
    let dependencyCheckInterval;
    let dependencyTimeout;
    ...
    function startDependencyCheck() {
        ...
        dependencyCheckInterval = setInterval(() => {
            ...
            if (checkDependencies()) {
                ...
                clearInterval(dependencyCheckInterval);
                console.warn('⚠️ Some dependencies missing, initializing with defaults');
                initializeApp();
            }
        }, 100);
        
        // Fallback timeout
        dependencyTimeout = setTimeout(() => {
            clearInterval(dependencyCheckInterval);
            console.warn('⚠️ Dependency timeout, initializing with available modules');
            initializeApp();
        }, 5000);
    }
    
    // Start checking for dependencies when DOM is ready
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', startDependencyCheck);
    } else {
